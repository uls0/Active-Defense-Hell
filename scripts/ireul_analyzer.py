...
def perform_triage(filepath, vt_api_key=None):
    """Realiza un análisis estático rápido y consulta reputación"""
    if not os.path.exists(filepath): return
    
    file_hash = get_file_hash(filepath)
    report_path = filepath + ".triage.txt"
    verdict = "UNKN_NEW_SAMPLE"
    ...
    if vt_api_key:
        try:
            ...
            if r.status_code == 200:
                ...
                verdict = f"MALICIOUS_DETECTION ({stats['malicious']} engines)"
        except: pass

    with open(report_path, "w") as f:
        f.write(f"HELL MALWARE TRIAGE\nSHA256: {file_hash}\nVerdict: {verdict}\n")
    
    print(f"[☣️] Triage completed for sample: {verdict}")
